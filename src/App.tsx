...
import {  
    ...
    deleteAllTodoRedux,  
    deleteLastTodoRedux,  
    updateAllCounterRedux,  
    updateCompletedCounterRedux,  
    showCompletedRedux,  
    showAllRedux  
} from './slice/todo';  
...
function App() {  
    ...
    function showTodos() {  
        if (todos.isOnlyCompletedShown) {  
            return (  
                <div data-testId='cypress-cardContainer' className='card-container'>  
                    {todos.checkedTodo.map((item: TTodo) =>   
                        <Card key={item.id} oneTodo={item} remove={remove} changeTodo={changeTodo} />  
                    )}  
                </div>  
            );  
        } else {  
